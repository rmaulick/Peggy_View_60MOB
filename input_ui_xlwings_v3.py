...
logging.basicConfig(
    filename="error_check_step.log", encoding="utf-8", level=logging.DEBUG
)

logging.debug("Step 1")

xlsm_file = "Input_UI.xlsm"
xw.Book(xlsm_file).set_mock_caller()
xlsm = xw.Book.caller()

logging.debug("Step 2")

seg_sheet = pd.read_excel("Input_UI.xlsm", sheet_name="Segment_Specific")
mob_sheet = pd.read_excel("Input_UI.xlsm", sheet_name="MOB_&_Overlay_Combined")
overlay_df = pd.read_excel(
    "Input_UI.xlsm", sheet_name="Overlay", skiprows=1, nrows=5
).iloc[:5, 1:18]
...
available_pcf_columns = [i for i in possible_pcf_columns if i in list(user_df.columns)]
...
for yr in yrs:
    for pcf in pcfs:
        for pcf_col in available_pcf_columns:
            ...
            ] = overlay_df.loc[(overlay_df["Project Curve Factor"] == yr), pcf].iloc[0]
...
wb = xw.Book("Input_UI.xlsm")

ws1 = wb.sheets["User_Assumptions"]
ws1["A1"].options(pd.DataFrame, header=1, index=True, expand="table").value = user_df


for pcf_col in possible_pcf_columns:
    final_df[pcf_col] = final_df[pcf_col].replace(np.NaN, 1)

final_df.update(user_df)

logging.debug("Step 10")

ws2 = wb.sheets["Final_Assumptions"]
ws2["A1"].options(pd.DataFrame, header=1, index=True, expand="table").value = final_df
